...
class SerializeTest(unittest.TestCase):
    ...
    def test_add_props_to_parser_ok_input_config(self):
        ...
        self.assertEqual(
            getattr(args, 'test_message.enum.int32_enum'), '1')
        self.assertEqual(
            getattr(args, 'test_message.params.bool_param'), 'True')
        self.assertEqual(
            getattr(args, 'test_message.params.int32_param'), '2')
        self.assertEqual(
            getattr(args, 'test_message.params.string_param'), 'bye')
    ...
    def test_fill_in_properties_from_args_ok_input_config(self):
        ...
        message: TestMessage = deserialize_from_namespace(
            args, TestMessage)

        self.assertEqual(
            message.enum.int32_enum, 1)
        self.assertEqual(
            message.params.bool_param, True)
        self.assertEqual(
            message.params.int32_param, 2)
        self.assertEqual(
            message.params.string_param, 'bye')
