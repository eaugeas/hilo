...
def _add_inputs_to_context(
        inputs: ProtoPipelineConfig.Input,
        context: Context
):
    for input in inputs:
        if input.WhichOneof('input') == 'source':
            ...
            context.put(
                'inputs/{0}'.format(input.source.id),
                source_channel)
        elif input.WhichOneof('input') == 'channel':
            context.put(
                'inputs/{0}'.format(input.channel.id),
                context.get(input.channel.url)
            )

        elif input.WhichOneof('input') is None:
            raise ValueError(
                'No input set for pipeline {0}'.format(
                    context.abs_current))
        else:
            raise ValueError(
                'Unknown input type {0} for pipeline {1}'.format(
                    input.WhichOneof('input'), context.abs_current))


def _add_outputs_to_context(
        outputs: ProtoPipelineConfig.Output,
        context: Context
):
    for output in outputs:
        if output.WhichOneof('output') == 'sink':
            context.put(
                'outputs/{0}'.format(output.sink.id),
                context.get(output.sink)
            )

        elif output.WhichOneof('output') == 'channel':
            context.put(
                'outputs/{0}'.format(output.channel.id),
...
class _StagesBuilder(_StepBuilder):
    ...
    def build(self, parent: Context) -> List[BaseNode]:
        return [
            StageBuilder(stage).build(parent.child(
                'stages/{0}'.format(stage.id)))
...
class _SequencePathBuilder(_SequenceBuilder):
    def __init__(self, path: Text):
        ...
        sequence = deserialize_from_file(
            path, ProtoPipelineConfig.Sequence)
...
class Builder(object):
    ...
    def build(self, context: Optional[Context] = None) -> TfxPipeline:
        ...
        context = context or Context(
            '/pipelines/{0}'.format(self._pipeline.id or 'global'))
        ...
        if self._pipeline.config.root_dir:
            ...
            context.put('config/root', context.get(
                'config/root', default=os.getcwd()))
