...
class Context(object):
    ...
    def child(self, current: Text) -> 'Context':
        return Context(current, self._parents + [self._current], self._context)
    ...
    def get(self, resource: Text, default: Optional[Any] = None) -> Any:
        ...
        for i in range(0, len(self._parents)):
            abs_resource = '/'.join(self._parents[0:len(self._parents) - i] +
                                    [resource])
        ...
        raise KeyError('Resource `{0}` not found in context'.format(resource))
    ...
    def put(self, resource: Text, o: Any):
        ...
        if not resource.startswith('/'.join(self._parents)):
            raise KeyError('Attempt to write to resource without permission '
